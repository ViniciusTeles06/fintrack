    ...
    <div style={{ maxWidth: 400, margin: "0 auto" }}>
      ...
      >
        {[0.25, 0.5, 0.75, 1].map((frac) => (
          <g key={frac}>
            <line
              x1={startX - 8} y1={chartH - frac * chartH}
        ...
        <line
          x1={startX - 8} y1={chartH}
          x2={startX + dados.length * gap} y2={chartH}
        ...
        {dados.map((d, i) => {
          ...
          return (
            <g key={d.mes}>
              <rect x={x} y={chartH - hR} width={barW} height={hR || 2} rx="4"
                fill={d.isAtual ? "#10b981" : "rgba(16,185,129,0.4)"} />
              <rect x={x + barW + 4} y={chartH - hD} width={barW} height={hD || 2} rx="4"
                fill={d.isAtual ? "#ef4444" : "rgba(239,68,68,0.4)"} />
              <text x={x + barW + 2} y={chartH + 18} textAnchor="middle"
                fontSize="11" fontWeight={d.isAtual ? "700" : "500"}
                fill={d.isAtual ? "var(--text-primary)" : "var(--text-secondary)"}>
                {d.mes}
              </text>
              {d.isAtual && (
                <text x={x + barW + 2} y={chartH + 32} textAnchor="middle"
                  fontSize="9" fill="#10b981" fontWeight="700">
                  atual
                </text>
              )}
            </g>
        ...
        <text x={startX + 14} y={chartH + 53} fontSize="10" fill="var(--text-secondary)">Receitas</text>
        <rect x={startX + 70} y={chartH + 44} width={10} height={10} rx="2" fill="#ef4444" />
        <text x={startX + 84} y={chartH + 53} fontSize="10" fill="var(--text-secondary)">Despesas</text>
      </svg>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  const dadosMeses = Array.from({ length: 4 }, (_, i) => {
    ...
    return { mes: MESES[mes], ano, receitas, despesas, isAtual: i === 3 };
  });
  ...
  return (
    <PageWrapper>
      ...
      <div className="grid-2 mt-16">
        <div className="card">
          <h2 style={{ fontWeight: 700, fontSize: 16, marginBottom: 16 }}>Gastos por categoria</h2>
          {categorias.length === 0 ? (
              ...
              return (
                <div key={cat} style={{ marginBottom: 14 }}>
                  <div className="flex items-center justify-between text-sm" style={{ marginBottom: 6 }}>
                    ...
                    <span style={{ color: "var(--text-secondary)" }}>{pct}% · {formatarMoeda(valor)}</span>
                  </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div className="card" style={{ textAlign: "center" }}>
            <p style={{ fontSize: 13, color: "var(--text-secondary)", marginBottom: 8 }}>Taxa de poupança</p>
            <p style={{ fontSize: 40, fontWeight: 800, color: taxaPoupanca >= 0 ? "#10b981" : "#ef4444" }}>
              {taxaPoupanca}%
            </p>
